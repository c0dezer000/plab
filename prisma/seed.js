// Plain JavaScript seed script (no tsx dependency)
...
async function main() {
  ...
  await prisma.beneficiarySiteEnrollment.upsert({
    ...
    create: { id: `${ben1}-${siteId}`, beneficiaryId: ben1, projectSiteId: siteId, status: 'ACTIVE', dateEnrolled: now, updatedAt: now }
  })
  await prisma.beneficiarySiteEnrollment.upsert({
    where: { id: `${ben2}-${siteId}` },
    update: { updatedAt: now },
    create: { id: `${ben2}-${siteId}`, beneficiaryId: ben2, projectSiteId: siteId, status: 'ACTIVE', dateEnrolled: now, updatedAt: now }
  })
